...
public class AWSSNSReceiver {
    // Shared queue for notifications from HTTP server
    private static BlockingQueue<Map<String, String>> messageQueue = new LinkedBlockingQueue<>();

    public static void receiveData(String accessKey, String secretKey) throws Exception {
        // Create a client
        ...
        server.setHandler(new AWSSNSHandler());
        server.start();
        ...
        SubscribeRequest subscribeReq = new SubscribeRequest()
                ...
                .withProtocol("http")
                .withEndpoint("http://52.14.19.42:" + port);
        service.subscribe(subscribeReq);

        System.out.println("Subscribed to the topic... now waiting...");
    ...
    // HTTP handler
    static class AWSSNSHandler extends AbstractHandler {
